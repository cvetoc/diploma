...
class Seq2SeqTransformer(nn.Module):
    def __init__(self,
        ...
        # self.decoder = nn.TransformerDecoder(decoder_layer, 16, decoder_norm) # , num_layers,

        # self.transformer = nn.Transformer(d_model=768, nhead=8, num_encoder_layers=2,
        ...
        self.linear_src = nn.Linear(768, len(self.tokenizer), bias=True)
    ...
    def forward_generation(self, batch):
        src, attention_mask, trg, _ = batch
        # trg = trg[:, :-1]
        # trg_am = trg_am[:, :-1]
        ...
        #         hid_src = self.model(input_ids=src, attention_mask=attention_mask).last_hidden_state.permute(1, 0, 2) # logits
    ...
    def eval_str(self, predicted_ids_list, target_tensor):
        ...
        actuals = target_tensor.detach().cpu().numpy()  # .reshape((-1))
